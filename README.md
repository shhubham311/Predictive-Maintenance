...
```bash
git clone https://github.com/shhubham311/Predictive-Maintenance.git
...
**Advanced Predictive Maintenance System** — Full-stack ML platform for aircraft engine health monitoring
- Architected end-to-end ML pipeline with XGBoost achieving **70.36% R² accuracy** (RMSE: 36.80 cycles) on NASA C-MAPSS dataset; integrated Optuna hyperparameter optimization with 20-trial sweeps
- Designed scalable microservices architecture: FastAPI backend for RUL inference (~100ms latency) + Streamlit dashboard with SHAP explainability plots for model interpretability
- Implemented hybrid anomaly detection system combining supervised regression with unsupervised Isolation Forest, enabling dual-layer safety monitoring (Normal/Critical/Anomaly states)
- Engineered domain-specific features including sensor interaction terms and time-series rolling/lag statistics; resolved inference constraints through stateless single-row processing logic
- Deployed containerized application (Docker, Railway, Render) with cross-platform optimization for Python 3.12 on Apple Silicon
